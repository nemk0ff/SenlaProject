...
class FeedFacadeImplTest {
  ...
  @Test
  void getNews_whenNoFriendsAndCommunities_thenReturnEmptyFeed() {
    when(userService.getUserByEmail(TEST_EMAIL_1)).thenReturn(testUser);
    when(friendRequestService.getFriendsByUser(testUser.getId())).thenReturn(List.of());
    when(communityMemberService.getAllByUser(testUser.getId())).thenReturn(List.of());
  ...
  @Test
  void getNews_whenOnlyFriendsExist_thenReturnFriendsPosts() {
    List<User> friends = List.of(friendUser);
    ...
    assertThat(((WallPostResponseDTO)result.get(0)).body()).isEqualTo(TEST_BODY);
  }

  @Test
  void getNews_whenOnlyCommunitiesExist_thenReturnCommunityPosts() {
    CommunityMember member = CommunityMember.builder()
        .community(testCommunity)
        .build();
    ...
    when(userService.getUserByEmail(TEST_EMAIL_1)).thenReturn(testUser);
    when(friendRequestService.getFriendsByUser(testUser.getId())).thenReturn(List.of());
    when(communityMemberService.getAllByUser(testUser.getId())).thenReturn(List.of(member));
    when(communityPostService.getAllPosts(testCommunity.getId())).thenReturn(List.of(communityPost));

    List<PostDTO> result = feedFacade.getNews(TEST_EMAIL_1);

    assertThat(result).hasSize(1);
    assertThat(((CommunityPostDTO)result.get(0)).body()).isEqualTo("Community post");
  }

  @Test
  void getNews_whenMultiplePosts_thenReturnSortedByDateDesc() {
    WallPost oldWallPost = WallPost.builder()
        .id(3L)
        .body("Old post")
        .createdAt(TEST_DATE.minusDays(1))
        .build();
    ...
    when(wallPostService.getByUser(friendUser.getId())).thenReturn(List.of(wallPost, oldWallPost));
    when(communityPostService.getAllPosts(testCommunity.getId())).thenReturn(List.of(communityPost));
